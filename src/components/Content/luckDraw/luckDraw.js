...
class LuckDraw extends Component{
    ...
    getParticipate() {
        axios.get(`/api/coupon/participate?page=${this.state.page}`, {})
        .then((res) => {
	...
	render(){
        let luckDraw_Content = this.state.data.map((value, index) => {
            ...
            return (
                <div key={index} className="luckDraw-item" onClick={this.goActivity.bind(this, value.id)}>
                    ...
                    <div className={ value.winUserId === 0 ? 'luckDraw-card-sign' : value.winUserId === JSON.parse(window.localStorage.getItem('user_info')).id ? 'greenbc luckDraw-card-sign' : 'redbc luckDraw-card-sign' }>
                        <span>{value.winMessage}</span>
                    </div>
                </div>
            )
        })
